<div>
    <div class="row mb-2 mb-xl-3">
        <div class="col-auto d-none d-sm-block">
            <h3><strong>{{$lang->data['wpform_leads']??'Product Inquiery'}}</strong></h3>
        </div>
<!-- 
                    ...
                    <table id="table" class="table table-striped table-bordered table-responsive mb-0">
                        <thead class="bg-secondary-light">
                            <tr>
                                ...
                                <th class="tw-15">{{$lang->data['date']??'Datum Tijd'}}</th>
                                <th class="tw-15">{{$lang->data['name']??' Product'}}</th>
                                <th class="tw-15">{{$lang->data['name']??'Naae'}}</th>
                                <th class="tw-15">{{$lang->data['address']??'Postcode'}}</th>
                                <th class="tw-15">{{$lang->data['email']??'E-mail'}}</th>
                                <th class="tw-15">{{$lang->data['telephone']??'Telefoonnummer'}}</th>
                                <th class="tw-20">{{$lang->data['address']??'Neem contact met mij op via'}}</th>
                                ...
                                @else
                                    ...
                                    <!--    @if(Auth::user()->can('edit_lead'))
                                        ...
                                        @if(Auth::user()->can('add_appointment'))
                                             <a href="{{route('admin.appointments')}}" class="btn btn-sm btn-success">{{$lang->data['make_quotation']??'Make Appointment'}}</a>
        ...
        <div class="modal-dialog modal-dialog-centered" role="document">
            <div class="modal-content">
                ...
                <div class="modal-body">
                    ...
                    <div class="mb-3">
                        <label class="form-label">{{$lang->data['address']??'Message'}}</label>
                        <textarea class="form-control resize-none" placeholder="{{$lang->data['address']??'Message'}}" wire:model="address"></textarea>
                    </div>
                </div>
